...
func (s *DeadLetterServiceImpl) Retry(ctx context.Context, id uint64) *exception.Exception {
	...
	entry.RetriedAt = &now
	rows, err := s.repo.DeadLetter().UpdateByMapWithTenantOrSystem(ctx, id, cctx.GetTenantID(ctx), map[string]any{
		"is_retried": true,
		"retried_at": now,
	})
	if err != nil {
...
func (s *DeadLetterServiceImpl) RetryAll(ctx context.Context, taskType string) (int, *exception.Exception) {
	...
	for _, entry := range entries {
		...
		entry.IsRetried = true
		entry.RetriedAt = &now
		rows, err := s.repo.DeadLetter().UpdateByMapWithTenantOrSystem(ctx, entry.ID, cctx.GetTenantID(ctx), map[string]any{
			"is_retried": true,
			"retried_at": now,
		})
		if err != nil {
...
func (s *DeadLetterServiceImpl) Delete(ctx context.Context, id uint64) *exception.Exception {
	...
	rows, err := s.repo.DeadLetter().HardDeleteWithTenantOrSystem(ctx, id, cctx.GetTenantID(ctx))
	if err != nil {
		return exception.InternalServerError.Append(err.Error())
	}
	if rows == 0 {
