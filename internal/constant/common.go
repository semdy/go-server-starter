...
const (
	...
	REDIS_EXPIRE_OF_AUTH_PERMISSIONS = 5 * time.Minute          // redis expire of auth permissions
	...
	REDIS_KEY_OF_AUTH_PERMISSIONS    = "auth:permissions:%d:%s" // redis key of auth permissions: tenantID:uniCode
...
func RedisKeyOfAuthPermissions(tenantID uint64, uniCode string) string {
	return fmt.Sprintf(REDIS_KEY_OF_AUTH_PERMISSIONS, tenantID, uniCode)
}
